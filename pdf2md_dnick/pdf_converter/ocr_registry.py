...
Usage:
    ...
    register_factory("my_model_mk", "My Model (mk)", lambda: MyModel(lang="mk"))
...
def register_model(key: str, label: str):
    ...
    Args:
        ...
        label: Human-readable name (e.g., "EasyOCR (en)")
...
# def register_factory(key: str, label: str, factory: Callable[[], "BasePDFModel"]) -> None:
#     """
#     Register a factory function that creates a model instance.
#
#     Useful for registering model variants (e.g., different languages).
#
#     Args:
#         key: Unique identifier for the model
#         label: Human-readable name
#         factory: Zero-argument callable that returns a BasePDFModel instance
#     """
#     k = key.strip()
#     if not k:
#         raise ValueError("Model key cannot be empty")
...
def safe_list_models() -> List[ModelInfo]:
    ...
    Filters out models with missing dependencies or API keys.
    """
    ok = []
