...
def convert_pdf(request):
    ...
    original_output = OriginalModelOutput.objects.create(
        model=model_key,
        text=text
    )
...
def autosave(request):
    ...
    if not output_id:
        return JsonResponse({"error": "Missing output_id"}, status=400)

    try:
        original_output = OriginalModelOutput.objects.get(id=output_id)
    except OriginalModelOutput.DoesNotExist:
        return JsonResponse({"error": "Original output not found"}, status=404)


    if text == original_output.text:
        UserChangedText.objects.filter(original_output=original_output).delete()
        return JsonResponse({"saved": False, "reason": "text_unchanged"})


    user_changed, created = UserChangedText.objects.update_or_create(
        original_output=original_output,
        defaults={"text": text}
    )

    return JsonResponse({"saved": True, "id": user_changed.id})
